data{
  ...
  real Y[n_obs];          //  vector outcome for training set
...
generated quantities {
  ...
  for(i in 1:n_obs) {
    ...
    if (Y[i] == 0) {
     ...
     log_lik[i]= log(k1 - k0) + beta_lpdf(Y[i] | sh1,sh2); 
    }
    
    int tmp = categorical_rng(probs);
    if (tmp == 3) {
      Yhat_train[i] = beta_rng(sh1, sh2);
    } else {
      Yhat_train[i] = tmp - 1;
    }
  }
  ...
  for(i in 1:n_obs2) {
    ...
    real  sh1 = exp((alpha[item2[i]]*theta[id2[i]] + beta[item2[i]] + omega[item2[i]])/2);
    real  sh2 = exp((-(alpha[item2[i]]*theta[id2[i]] + beta[item2[i]]) + omega[item2[i]])/2);
    ...
    if (Y2[i] == 0) {
      ...
      log_lik2[i]= log1m(k1);                       
       ...
       log_lik2[i]= log(k1 - k0) + beta_lpdf(Y2[i] | sh1,sh2);  
    }
    
    int tmp = categorical_rng(probs);
    if (tmp == 3) {
      Yhat_test[i] = beta_rng(sh1, sh2);
    } else {
      Yhat_test[i] = tmp - 1;
    }
  }
