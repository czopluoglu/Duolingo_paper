...
# True Item parameters for 10 hypothetical items
  ...
  ipar <- data.frame(gamma0 = rep(c(-2,-.9,-2.6,-.8,-3,-2.5,-2.9,-2.6,-1.8,2),5),
                     ...
                     delta  = rep(c(0.5,0.4,0.5,0.7,0.7,0.5,0.4,0.7,0.5,0.4),5))
...
for(p in 1:N){
  for(i in 1:nrow(ipar)){
    ...
    if(C==2){
      ...
      mu_  <- ipar$alpha[i]*theta[p]+ipar$beta[i]
      sig_ <- sqrt(ipar$delta[i]) 
      
      d[p,i] <- plogis(rnorm(1,mu_,sig_))
...
write.csv(d_long,'./data/sb_long.csv',row.names=FALSE)
write.csv(d,'./data/sb_wide.csv',row.names=FALSE)
